...
class OrdersController extends ApiController
{
    ...
    public function getAllOrders(Request $request){
            $orders = Order::getAllOrders($request->input('search'))->paginate(15);
    ...
    public function getOrdersCount(Request $request){
        $count = Order::getAllOrders($request->input('search'))->count();;
    ...
    public function getNeededDocs(Order $order){
        if ($this->guard()->user()->cant('using',$order))
            throw new AccessDeniedHttpException('Your can`t view info of not your order');
